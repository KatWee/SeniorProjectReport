...
\begin{enumerate}
  \item \textbf{Failed task:} The result of failed and finished tasks for each framework before and after applied this policy is shown in \textbf{Figure}~\ref{fig:finishFail} and \textbf{Figure}~\ref{fig:finishFailFDP}. The number of Chronos failed tasks was decreased, but in the other hand, number of spark failed tasks was increased. In \textbf{Figure}~\ref{fig:finishFailRate} shows that growth rate of failed tasks is upward when used this policy. The slope was increased from 0.195 to 0.251. So, this policy cannot reduce the number of failed tasks in every framework and increase its failure rate.
  \begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
