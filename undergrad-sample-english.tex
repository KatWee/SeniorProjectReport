...
\begin{figure}[!h]\centering
  \setlength{\fboxrule}{0mm} % can define this in the preamble
  \setlength{\fboxsep}{0cm}
  \fbox{\includegraphics[width=10cm]{./image/NN.png}}
...
\begin{figure}[!h]\centering
  \setlength{\fboxrule}{0mm} % can define this in the preamble
  \setlength{\fboxsep}{0cm}
  \fbox{\includegraphics[width=10cm]{./image/chronos.png}}
...
\begin{figure}[!h]\centering
  \setlength{\fboxrule}{0mm} % can define this in the preamble
  \setlength{\fboxsep}{0cm}
  \fbox{\includegraphics[width=6cm]{./image/kafka.png}}
...
\begin{figure}[!h]\centering
  \setlength{\fboxrule}{0mm} % can define this in the preamble
  \setlength{\fboxsep}{0cm}
  \fbox{\includegraphics[width=14cm]{./image/SequenceDiagram.png}}
...
\begin{figure}[!h]\centering
  \setlength{\fboxrule}{0mm} % can define this in the preamble
  \setlength{\fboxsep}{0cm}
  \fbox{\includegraphics[width=14cm]{./image/ArchitectureDiagram.png}}
...
\begin{enumerate}
  ...
  \begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=12cm]{./image/flowDiagram.png}}
...
\hspace{10mm}In each cluster, data will be divided into 80\% training dataset and remaining 20\% testing dataset. The measure metric is accuracy, precision, recall and error. When users submit task, NKR-scheduler will request metric information from Mesos, allocate resource to run tasks, profile the task to data cluster and use that cluster model to predict success rates of their task shown in \textbf{Figure}~\ref{fig:flowDiagramPredict}.
  ...
  \begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=15cm]{./image/flowDiagramPredict.png}}
...
\begin{figure}[!h]\centering
  \setlength{\fboxrule}{0mm} % can define this in the preamble
  \setlength{\fboxsep}{0cm}
  \fbox{\includegraphics[width=13cm]{./image/dataPipeline.png}}
...
\hspace{10mm} In this chapter details about experimental setup, results, and discussion.  \textbf{Topic 4.1} details about control variables in cluster for this experiment and results of simulated data.  \textbf{Topic 4.2} results of data processing and model for predict success rate.  \textbf{Topic 4.3} and  \textbf{Topic 4.4} are the results from each policy.  \textbf{Topic 4.5} is the result when used 2 policies together. Lastly,  \textbf{Topic 4.6} discusses about the result of  \textbf{Topic 4.3} to  \textbf{Topic 4.5} and how to improve them.

\section{Setup cluster, framework application, and parameter}
\hspace{10mm}For our experiment setup, we had setup a cluster inside a cloud provider which called AWS. This cluster consists of 3 nodes with 6 CPUs, 8.4 GB of memory, and 135 GB of disk for each node as shown in \textbf{Figure}~\ref{fig:resourcesCluster}. We setup this cluster with 3 widely known Mesos Framework, Spark Marathon, and Chronos, and conducted the way to test our setup by simulated data. We had randomly varied the resources of each task from the range that each task can provide. 
\begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
...
\begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=14cm]{./image/NumberTask.png}}
    \caption{Number of tasks for each framework}\label{fig:NumberTask}
\end{figure}

\newpage

\hspace{10mm}We got the example of cluster metric data as in \textbf{Figure}~\ref{fig:exampleData} after run sample job in cluster and gathering cluster metric data. Each row of cluster metric data was captured every 5 seconds. , and each column of cluster metric data was an information of cluster, for example, CPU utilization, memory utilization, number of finished tasks, number of failed tasks and other information.
\begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
...
\hspace{10mm}We clustered metric data by using k-means and used elbow method to find optimal k parameter which is number of data cluster in k-means. The result is shown in \textbf{Figure}~\ref{fig:elbow}, from the graph k = 2 was chosen because at this point the distortion start to decrease in linear form. Therefore, the cluster metric data was separate into 2 clusters by k-means and the result is shown in \textbf{Figure}~\ref{fig:cluster}. But these 2 data clusters were not separated clearly, so we considered to use the whole data to build model instead.
\begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=10cm]{./image/elbow.png}}
    \caption{Elbow method for optimal k}\label{fig:elbow}
\end{figure}
\begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=10cm]{./image/cluster.png}}
    \caption{Data clustering}\label{fig:cluster}
\end{figure}

\subsection{Random forest model}  
\hspace{10mm}We trained random forest model with these following parameters; max\_depth = 10, random\_state = 10 and max\_features = 10. Result is shown in \textbf{Table}~\ref{tbl:modelResult}. The accuracy of this model is 0.832, precision is 0.85 and recall is 0.98.
\begin{table}[!h]
  \caption{Result from random forest model.}\label{tbl:modelResult}
  \begin{tabular}{|l|l|l|}
    \hline
    & \textbf{Fail (predicted)} & \textbf{Finish (predicted)} \\
    \hline
    \textbf{Fail (actuated)} & 317 & 6 \\
    \hline
    \textbf{\textbf{Fail (actuated)}} & 58 & 0 \\ 
    \hline             
  \end{tabular}
\end{table}

\section{Policy 1: First Demand Share Policy (FDP)}
\hspace{10mm}For the FDP, we used the same setup as mentioned in  \textbf{Section 4.1}. We have developed NKR to receive tasks, and then it considers based on the first policy. We also submitted tasks as \textbf{Table}~\ref{tbl:FDPTaskConfiguration}.
\begin{table}[!h]
  \caption{FDP task configuration.}\label{tbl:FDPTaskConfiguration}
  \begin{tabular}{|l|l|l|}
    \hline
    \textbf{Framework} & \textbf{Number of tasks} & \textbf{Arrival Rate (sec)} \\
    \hline
...
The result of submitted task is shown in \textbf{Figure}~\ref{fig:FDPNumberTask}.
\begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=14cm]{./image/FDPNumberTask.png}}
    \caption{Number of tasks for each framework after using FDP}\label{fig:FDPNumberTask}
\end{figure}

\hspace{10mm}From \textbf{Figure}~\ref{fig:NumberTask}, Marathon and Chronos could not launch a fair number of tasks because Spark could hold on to offer more than the others. After we compared with normal Mesos, we found a little improvement of fairness as shown in \textbf{Figure}~\ref{fig:FDPNumberTask}. Each framework had almost the same approximate number of tasks, which is 1.5. We also calculated different average, the value that help to compare between both policies, first policy was following with these values (1.5919, 1.5349, 1.5704). After calculated, the result is 0.1867. So, this policy can improve fairness compared to the default policy 0.0380. Then it comes to the result of fairness improvement by 79.6465 \%.

We also considered other matrices after applying this policy.

\begin{enumerate}
  \item \textbf{Failed task:} The result of failed and finished tasks for each framework before and after applied this policy is shown in \textbf{Figure}~\ref{fig:finishFail} and \textbf{Figure}~\ref{fig:finishFailFDP}. The number of Chronos’ failed tasks was decreased, but in the other hand, number of spark’s failed tasks was increased. In \textbf{Figure}~\ref{fig:finishFailRate} shows that fail rate is faster when used this policy. The slope was increased from 0.195 to 0.251. So, this policy cannot reduce the number of failed task in every framework and increase its failure rate.
  \begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=10cm]{./image/finishFail.png}}
    \caption{Number of failed and finish tasks before using FDP}\label{fig:finishFail}
  \end{figure}
  \begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=10cm]{./image/finishFailFDP.png}}
    \caption{Number of failed and finish task after using FDP}\label{fig:finishFailFDP}
  \end{figure}
  \begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=14cm]{./image/finishFailRate.png}}
    \caption{Growth rate of fail task before and after using FDP}\label{fig:finishFailRate}
  \end{figure}
  
  \item \textbf{CPU and memory utilization:} Comparing CPU and memory utilization of cluster framework before and after used this policy, the averages of CPU and memory utilization is shown in \textbf{Table}~\ref{tbl:FDPCPUmem}. From the table, both CPU and memory utilization average were decreased. The amounts of CPU and memory utilization before and after used this policy in each time is shown in \textbf{Figure}~\ref{fig:FDPCPU} and \textbf{Figure}~\ref{fig:FDPmem}. 
  \begin{table}[!h]
  \caption{average of CPU and memory utilization before and after using FDP.}\label{tbl:FDPCPUmem}
  \begin{tabular}{|l|l|l|}
    \hline
    & \textbf{Before} & \textbf{After} \\
    \hline
    \textbf{CPU} & 4.982 & 4.755 \\
    \hline
    \textbf{\textbf{Memory}} & 3,408 & 3,348 \\ 
    \hline    
  \end{tabular}
  \end{table}
  \begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=14cm]{./image/FDPCPU.png}}
    \caption{CPU utilization before and after using FDP}\label{fig:FDPCPU}
  \end{figure}
  \begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=14cm]{./image/FDPmem.png}}
    \caption{Memory utilization before and after using FDP}\label{fig:FDPmem}
  \end{figure}
  
  \item \textbf{System load:} The result of system load in this cluster before and after used this policy is shown in \textbf{Figure}~\ref{fig:FDPload}. System load average before using FDP is 2.142 and after is 1.943. So, system is less busy when applied this policy. 
\begin{figure}[!h]\centering
    \setlength{\fboxrule}{0mm} % can define this in the preamble
    \setlength{\fboxsep}{0cm}
    \fbox{\includegraphics[width=14cm]{./image/FDPload.png}}
    \caption{System load before and after using FDP}\label{fig:FDPload}
  \end{figure}
\end{enumerate}

\hspace{10mm}Therefore, Policy 1 (FDP) can improve fairness, resources utilization, and system load. By the way, the number of failed tasks is decreased only in some frameworks and failure rate is increased. 

\section{Policy 2: Success rate prediction}
\section{Policy 1 and 2}
\section{Discussion}
